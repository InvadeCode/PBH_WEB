...
const DynamicSciArtGrid = ({ content }) => {
  const sections = [
    content.about   && { label: 'About the Brand',    text: [content.about] },
    content.problem && { label: 'The Problem',         text: [content.problem] },
    (content.solution1 || content.solution2) && {
      label: 'Creative Solution',
      text: [content.solution1, content.solution2].filter(Boolean),
    },
  ].filter(Boolean);
  ...
  return (
    <section className="w-full py-20 lg:py-28 px-6 md:px-12">
      <div className="max-w-4xl mx-auto flex flex-col gap-16 md:gap-24">
        {sections.map((section, i) => (
          ...
          >
            <p className="text-sm md:text-base uppercase tracking-[0.4em] text-cyan-400 font-bold mb-5 font-primary">
              {section.label}
            </p>
            {section.text.map((para, j) => (
              <p key={j} className="text-[17px] md:text-[19px] font-secondary leading-[1.8] text-white/90 mb-4 last:mb-0">
                {para}
              </p>
            ))}
          </motion.div>
